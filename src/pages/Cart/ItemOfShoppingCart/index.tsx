...
import {
  ...
  Decrement,
  Increment,
} from '../../Home/Coffees/CardCoffee/styles'
import { ButtonRemove, ItemsOfCard } from './styles'

export function ItemOfShoppingCart() {
  const { itemsInCard } = useContext(CardContext)
  const [amountCoffeeOfShoppingCart, setAmountCoffeeOfShoppingCart] =
    useState(1)

  function decrementAmountCoffeeOfShoppingCart() {
    setAmountCoffeeOfShoppingCart((state: number) => {
      if (state >= 2 && state <= 9) {
        state = state - 1
      }
      return state
  ...
  function incrementAmountCoffeeOfShoppingCart() {
    setAmountCoffeeOfShoppingCart((state: number) => {
      if (state >= 1 && state <= 8) {
        state = state + 1
      }
  ...
  return (
    <>
      {itemsInCard.map((items: CardProps) => {
        return (
          <ItemsOfCard key={items.id}>
            <img src={items.src} alt="" />
            <div>
              <p>{items.subTitle}</p>
              <div>
                <BoxInputsAndButtons>
                  <Decrement onClick={decrementAmountCoffeeOfShoppingCart}>
                    -
                  </Decrement>
                  <input
                    type="number"
                    min={1}
                    max={9}
                    value={amountCoffeeOfShoppingCart}
                    onChange={(e) =>
                      setAmountCoffeeOfShoppingCart(Number(e.target.value))
                    }
                    required
                  />
                  <Increment onClick={incrementAmountCoffeeOfShoppingCart}>
                    +
                  </Increment>
                </BoxInputsAndButtons>
                <ButtonRemove>
